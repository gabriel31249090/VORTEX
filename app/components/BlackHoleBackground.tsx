 ...
 *  - Auto-detecta mobile e reduz partículas automaticamente
 *  - Pausa o render loop quando a aba está escondida (Page Visibility API)
 *  - Cleanup completo de geometria, materiais e WebGL context
 *  - Respeita prefers-reduced-motion (frame estático, sem loop)
 *  - Default reduzido de 8000 → 4500 partículas
 *  - DPR clamp mais agressivo em telas mobile
 *
 ...
 * Agora: até 4500 partículas custam o mesmo que 500 no CPU.
 *
 * Uso:
 ...
 *     <BlackHoleBackground intensity={0.4} />
...
type BlackHoleBackgroundProps = {
  className?: string
  /** Total de partículas (todas as camadas). Default 4500 (auto-reduz em mobile). */
...
void main() {
  ...
  if (t < 0.5) {
    ...
    vec3 cool = uDiscColor * 0.18 + vec3(0.3, 0.08, 0.0);
    vec3 hotC = uDiscColor * 1.6 + vec3(0.5, 0.5, 0.3);
...
export default function BlackHoleBackground({
  ...
  useEffect(() => {
    ...
    // Auto-detect mobile e reduz partículas + DPR pra performance
    const mobile = isMobile()
    const effectiveCount = particleCount ?? (mobile ? 1800 : 4500)
    ...
    const renderer = new THREE.WebGLRenderer({
      antialias: !mobile, // desliga antialias em mobile pra ganhar fps
      alpha: true,
    ...
    // ── EVENT HORIZON ─────────────────────────────────────
    const horizonGeo = new THREE.SphereGeometry(1.6, mobile ? 32 : 64, mobile ? 32 : 64)
    const horizonMat = new THREE.MeshBasicMaterial({ color: 0x000000 })
    const horizon = new THREE.Mesh(horizonGeo, horizonMat)
    scene.add(horizon)

    // ── PHOTON SPHERE (dois anéis) ────────────────────────
    const photonGeo = new THREE.TorusGeometry(2.3, 0.08, 16, mobile ? 120 : 220)
    ...
    const photon2Geo = new THREE.TorusGeometry(2.9, 0.04, 12, mobile ? 100 : 180)
    ...
    // ── PARTICLE SYSTEM ───────────────────────────────────
    const coreCount = Math.floor(effectiveCount * 0.05)
    const discCount = Math.floor(effectiveCount * 0.72)
    const jetTotal = effectiveCount - coreCount - discCount
    ...
    for (let k = 0; k < discCount; k++, i++) {
      ...
      aRadius[i] = r
      aAngle0[i] = Math.random() * Math.PI * 2
      aHeight[i] = (Math.random() - 0.5) * (0.4 + Math.random() * 0.4) / (r * 0.3 + 0.4)
      aType[i] = 1
    ...
    // ── STARFIELD ─────────────────────────────────────────
    const starCount = mobile ? 250 : 700
    ...
    function render() {
      ...
      scene.rotation.y = Math.sin(material.uniforms.uTime.value * 0.05) * 0.18
      scene.rotation.x = Math.cos(material.uniforms.uTime.value * 0.04) * 0.06
    ...
    function tick() {
      ...
      if (isVisible) {
        ...
        // Pausa total: nem requestAnimationFrame, nem render.
        // Vai reentrar no próximo visibilitychange.
        frameId = 0
      }
    }
    ...
    function handleVisibility() {
      ...
      if (isVisible && running && frameId === 0) {
        // Reset clock pra não ter salto enorme
        clock.start()
        tick()
      }
    }
    ...
    // Resize com debounce pra não engasgar durante drag de janela
    ...
    return () => {
      ...
      // Force WebGL context loss pra liberar memória de GPU
